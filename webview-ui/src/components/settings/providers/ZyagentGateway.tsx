...
export const ZyagentGateway = ({
	...
	// 在组件挂载时自动设置默认的 baseUrl，确保模型列表能够正常获取
	useEffect(() => {
		const defaultBaseUrl = homepages.llmApiKey
		if (!apiConfiguration?.openAiBaseUrl || apiConfiguration.openAiBaseUrl !== defaultBaseUrl) {
			setApiConfigurationField("openAiBaseUrl", defaultBaseUrl)
		}
	}, [apiConfiguration?.openAiBaseUrl, setApiConfigurationField, homepages.llmApiKey])
